...
module.exports = async (req, res) => {
  ...
  try{
    ...
    if(req.method==='POST'){
      ...
      // Passa pela CASCATA (Groq → Cerebras → Gemini → DeepSeek → OpenAI) — este arquivo
      // é CommonJS e a cascata é ESM, por isso o import é dinâmico (await import).
      let reply=null;
      if(autor==='elias'){
        try{
          ...
          const { iaTexto } = await import('./_lib/ia.js');
          const rr = await iaTexto({ sys:SYS, messages:msgs, temperature:0.5, max_tokens:500, tag:'chat' });
          const ans = rr.texto;
          if(ans){
